...
def test_unexpected_sender_failure_is_not_misclassified_as_retryable_network() -> None:
    failure = RuntimeError()

    with pytest.raises(RuntimeError) as raised:
        GitHubTransport(_TOKEN, sender=_FakeSender(failure=failure)).request(
            GitHubMethod.POST,
            "/repos/owner/repo/git/refs",
            {},
        )
...
class _DetachOnResponseHTTPSConnection(_FakeHTTPSConnection):
    def __init__(self, response: _FakeHTTPResponse) -> None:
        super().__init__(response)
        self.sock = None
        self.response_socket = _FakeSocket()

    def connect(self) -> None:
        self.connected = True
        self.sock = self.response_socket
...
def test_stdlib_sender_keeps_reading_when_http_client_detaches_close_response(
    monkeypatch: MonkeyPatch,
) -> None:
    response = _FakeHTTPResponse(b'{"ok":true}')
    connection = _DetachOnResponseHTTPSConnection(response)
    ...
    def factory(
        ...
        *,
        timeout: float,
        context: ssl.SSLContext,
    ) -> _DetachOnResponseHTTPSConnection:
    ...
    result = GitHubTransport(_TOKEN).request(GitHubMethod.GET, "/rate_limit")

    assert result.body == {"ok": True}
    assert connection.sock is None
    assert connection.response_socket.timeouts
    assert response.isclosed() is True
    assert connection.closed is True


@pytest.mark.parametrize(
    ("method", "body", "ambiguous"),
    [
        (GitHubMethod.GET, None, False),
        (GitHubMethod.POST, {}, True),
    ],
)
def test_stdlib_sender_rejects_truncated_declared_response_and_closes_it(
    monkeypatch: MonkeyPatch,
    method: GitHubMethod,
    body: dict[str, GitHubJSONValue] | None,
    ambiguous: bool,
    ...
    with pytest.raises(GitHubTransportError) as raised:
        GitHubTransport(_TOKEN).request(method, "/rate_limit", body)
...
def test_stdlib_sender_closes_detached_response_after_read_failure(
    monkeypatch: MonkeyPatch,
) -> None:
    response = _FailingHTTPResponse(b'{"ok":true}')
    connection = _DetachOnResponseHTTPSConnection(response)
    ...
    with pytest.raises(GitHubTransportError) as raised:
        GitHubTransport(_TOKEN).request(GitHubMethod.GET, "/rate_limit")
